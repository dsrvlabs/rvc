...
| KM-2 | P1 | `crates/keymanager-api` | Doppelganger cancel-token map overwritten without cancellation on delete+re-import. | GREEN-landed | `2ee145a` (2.12) | `f94c0bd`,`4e0cfa3` (2.12) | `crates/keymanager-api/tests/km2_cancel_token_race.rs` | `ForwardWindowMachine` (2.6) | Phase 2 / 2.12 (reviewer APPROVED, bug-hunter re-verified). PRD KM-2 (a)+(b)+(c)+(d) all satisfied: (a) import insert cancels displaced token under the `cancel_tokens` guard; (b) new `doppelganger_state_lock` held across delete's keystore-removal + token-cancel AND import's add+monitor+insert; (c) window-elapsed task prunes its own entry; (d) race regression suite (4 tests). Review fix `4e0cfa3` ALSO closed a residual `select!` TOCTOU (stale timer could enable a re-imported key on a same-tick tie) by re-checking `is_cancelled()` under `doppelganger_state_lock` in the timer branch, and made the delete-path token-cancel unconditional. Full `ForwardWindowMachine::cancel` unification deferred to FUP-3 (no live-ticked machine in this path). Residual non-blockers → FUP-8. |
...
| FUP-8 | 2.12 re-review (bug-hunter) | Two non-blocking items on the KM-2 fix (race is fully closed; these are quality/hygiene). (1) **Test RED-power**: `km2_stale_timer_cannot_enable_after_displacement` relies on a both-branches-ready `tokio::select!` tie, which `select!` resolves without bias (~51% chance of exercising the sleep arm per run on broken code) — so it is NOT deterministic-RED (the 2.12 fix commit message overstates "deterministically"; it failed on the observed run but is ~50%/run). The fix is correct regardless and passes 100% on fixed code; the structural test `km2_concurrent_delete_reimport_no_stale_enable` covers the import-displacement invariant deterministically. (2) **Delete state-hygiene** (pre-existing): the delete `Ok(false)` (NotFound) / `Err` arms cancel the token (now unconditional, so no stale-enable) but do not call `remove_validator`/`stop_monitoring`, leaving lingering validator/gate entries on a rare failed-delete path. Not a slashing-safety issue. | Test-hardening follow-up: make the finding-1 regression deterministic (force the timer-arm-with-cancel-ready ordering, or assert the cancellation-ordering invariant directly) + fix the commit-message claim in notes; and reconcile validator/gate state on all delete arms. Low priority. |
