...
| DVT-1 | P0 | `bin/rvc-signer`, `crates/slashing` | DVT `ScopedSlashingDb` keyed by peer CN → two CNs double-sign one validator/slot. | GREEN-landed | `1658149` | `c4b17d4`,`6a41ec1` | `crates/slashing/tests/pubkey_scope_cross_cn.rs`, `crates/slashing/tests/migration_v3_cases.rs` | `PubkeyScopedDb` (2.4) | Phase 2 / 2.4 (schema). v2→v3 migration: pubkey+gvr unique indices; client_cn audit-only; 5-case row-pair resolution; every insert path writes non-NULL gvr (index enforces); fails-closed if NULL-gvr rows + no pinned GVR. Call-site rekey + ScopedSlashingDb delete = 2.5. |
...
| CN-1 | P1 | `bin/rvc-signer`, `crates/slashing` | Main signer namespaces slashing per CN; same key under two CNs gets no cross-CN check. | GREEN-landed | `1658149` | `c4b17d4`,`6a41ec1` | `crates/slashing/tests/pubkey_scope_cross_cn.rs` | `PubkeyScopedDb` (2.4) | Phase 2 / 2.4 (closed with DVT-1 — same schema fix). Cross-CN double-block & double-vote now rejected. |
