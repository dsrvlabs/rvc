...
| D-3 | P0 | `crates/rvc`, `crates/validator-store` | Doppelganger gate consulted only on attestation; block/sync/aggregate fail-open. | In progress | `f13b657` | `0dd44bc`,`cae7518` (2.9a) | `crates/signer/tests/gate_*.rs` | `SigningEnablement` (doppelganger), `PubkeyScopedDb`, `ForwardWindowMachine` | Phase 2 / 2.9a (slashable SigningGate skeleton + ValidatorLockMap + sign timeout, DONE) → 2.9b (non-slashable + FailClosedDefault) → 2.10a/2.10b (route all call sites) → 2.11 (rename + unknown→false). Gate not yet wired to handlers. |
