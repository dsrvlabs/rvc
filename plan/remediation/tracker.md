...
| D-3 | P0 | `crates/rvc`, `crates/validator-store` | Doppelganger gate consulted only on attestation; block/sync/aggregate fail-open. | In progress | `f13b657`,`d74e034` | `0dd44bc`,`cae7518` (2.9a); `4e00812`,`4f25d1a` (2.9b) | `crates/signer/tests/gate_*.rs` | `SigningEnablement` (doppelganger), `PubkeyScopedDb`, `ForwardWindowMachine`, `FailClosedDefault` | Phase 2 / 2.9a (slashable skeleton + ValidatorLockMap + sign timeout) + 2.9b (all 7 non-slashable paths, aggregate chain-of-custody doc for SS-2/SS-3 @4.5, FailClosedDefault unknown→fail-closed) DONE — SigningGate complete. NOT yet wired to handlers → 2.10a/2.10b (route all call sites; resolve FUP-3 ForwardWindowMachine index→pubkey-hex) → 2.11 (rename + unknown→false). |
