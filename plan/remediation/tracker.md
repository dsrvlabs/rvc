...
| D-3 | P0 | `crates/rvc`, `crates/validator-store` | Doppelganger gate consulted only on attestation; block/sync/aggregate fail-open. | GREEN-landed (a); 2.11 pending (b) | `f13b657`,`d74e034`; `589f241` (2.10b) | `0dd44bc`,`cae7518` (2.9a); `4e00812`,`4f25d1a` (2.9b); `789fd07`,`a1c3abc` (2.10a); `7b73b03`,`8cf3b66` (2.10b) | `crates/signer/tests/gate_*.rs`, `crates/signer/tests/no_direct_composite_signer_outside_signer.rs`, orchestrator per-path D-3 tests (coordinator/sync_committee/aggregation) | `SigningEnablement` (doppelganger), `PubkeyScopedDb`, `ForwardWindowMachine`, `FailClosedDefault` | Phase 2 / 2.9a+2.9b: SigningGate complete. 2.10a: every `rvc-signer` typed handler routed through gate (SS-2/SS-3 closed early). **2.10b DONE**: orchestrator gates EVERY signing path per-pubkey (block@coordinator.rs:614, sync msg+contribution@sync_committee.rs:314, aggregate@aggregation.rs:98) via infallible `pubkey.to_bytes()` (fail-open hex-decode removed at all sites); grep-gate (`no_direct_composite_signer_outside_signer.rs`) now Live — no signing path may bypass `crates/signer`. Remaining: 2.11 (rename `is_attesting_enabled`→`is_signing_enabled` + unknown-pubkey default true→false fail-closed); FUP-3 (ForwardWindowMachine index→pubkey-hex live wiring) still tracked separately. |
