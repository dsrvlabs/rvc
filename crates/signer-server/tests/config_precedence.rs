...
use signer_server::backend::basic::BasicSigner;
use signer_server::backend::SigningBackend;
use signer_server::config::{self, Backend, ServeArgs};
...
async fn test_metrics_endpoint_serves_prometheus_text() {
    ...
    let (handle, bound_addr) =
        signer_server::metrics::serve_metrics(addr, Arc::clone(&metrics)).await.unwrap();
...
fn test_dry_run_invalid_tls_cert_detected() {
    ...
    let tls = signer_server::grpc_tls::TlsConfig::new(
...
fn test_audit_entry_fields_populated() {
    let entry = signer_server::audit::AuditEntry {
        timestamp: signer_server::audit::now_rfc3339(),
...
fn test_audit_extract_cn_without_tls_returns_unknown() {
    ...
    let cn = signer_server::audit::extract_client_cn(&request);
